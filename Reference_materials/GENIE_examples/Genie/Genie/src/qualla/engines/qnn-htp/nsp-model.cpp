...
bool QnnNspModel::initializeModel(void) {
  ...
  if (!m_qnnApi->populateGraphBinaryInfo(model_filelist, graph_switching, m_use_mmap, m_systemLib)) {
...
bool QnnNspModel::validateModel() {
  ...
  for (auto& [variant_spec, variant] : m_nsp_graphs.back().variants) {
    if ((tt = variant->getOutput(m_layerNames[LayerType::CROSS_ATTN_STATES])) != nullptr) {
      if (!m_cross_attention || (m_modelArchitectureType != ModelArchitectureType::ENCODER)) {
        throw std::runtime_error("Unexpected output: " + m_layerNames[LayerType::CROSS_ATTN_STATES]);
      }
      break;
    }
  ...
  if ((m_modelArchitectureType == ModelArchitectureType::ENCODER) && m_cross_attention && m_pooled_output) {
  ...
  for (auto& [variant_spec, variant] : m_nsp_graphs.back().variants) {
    ...
    if (m_modelArchitectureType == ModelArchitectureType::ENCODER) {
      if (m_cross_attention) {
        if ((tt = variant->getOutput(m_layerNames[LayerType::CROSS_ATTN_STATES])) == nullptr)
          errors.push_back(
              {variant->graph_name, m_layerNames[LayerType::CROSS_ATTN_STATES], "Tensor not found"});
        else {
...
bool QnnNspModel::initializeTensorPointers() {
  ...
  for (auto& variant : m_variant_list) {
    ...
    status &= updateTensorPointer(variant, m_layerNames[LayerType::CROSS_ATTN_STATES], t_cross_attn_states);
    status &= updateTensorPointer(variant, m_layerNames[LayerType::CROSS_ATTN_MASK], t_cross_attn_mask);
    status &= updateTensorPointer(variant, m_layerNames[LayerType::FULL_TEXT_ROW_MASK], t_full_text_row_mask);
  }
...
bool QnnNspModel::setupInputEmbeddings(const InferenceStep& step,
  ...
  uint8_t* embed_ptr            = reinterpret_cast<uint8_t*>(getBuffer(t_input_ids));
  const uint8_t* eagle_embed_in = reinterpret_cast<const uint8_t*>(eagle_embed.data());
  uint16_t embedDraft           = getEmbeddingBufferSize();
  size_t count                  = eagle_embed.size() / embedDraft;
  size_t embed_offset_len       = embedDraft;
  size_t feature_len            = m_embedding_length * sizeof(uint16_t);
  size_t feature_offset_len     = feature_len;
  size_t embed_len              = embedDraft;
  uint16_t increm               = m_embedding_length;
  size_t embedFactor            = 1;
  if (m_embedding_length) {
    embedFactor = embedDraft / m_embedding_length;
  }

  embed_ptr += start_idx * embed_offset_len * embedFactor;

  uint8_t* offset_ptr      = embed_ptr + embed_len;
  uint16_t* feature_in_ptr = reinterpret_cast<uint16_t*>(offset_ptr);
  ...
  if (isDualHead) {
    ...
    embed_ptr += start_idx * embed_offset_len;
  ...
  if (selected.size() == 0) {
    if (eagle_extra_feature == nullptr) {
      ...
      const uint8_t* embed_data = (eagle_embed_in);
    ...
    embed_ptr += embed_len * embedFactor;
    feature_in_ptr += feature_offset_len / offset_divide_len;

    for (size_t i = 1; i < static_cast<size_t>(step.variant); i++) {
      // update the pointer to next token embedding data
      const uint8_t* embed_data = eagle_embed_in + (i * increm * embedFactor);
      // feature data of one before token to copied
      const uint16_t* feature_data =
          eagle_feature_in + (i - 1 - in_buf_offset) * feature_len / sizeof(uint16_t);
      // concat the embedding and feature vector data
      std::memcpy(embed_ptr, embed_data, embed_len);
      std::memcpy(feature_in_ptr + embed_len / embedFactor, feature_data, feature_len);
      embed_ptr += embed_len * embedFactor;
      feature_in_ptr += feature_offset_len / offset_divide_len;
    }
    ...
    const uint8_t* embed_data    = nullptr;
    ...
    for (uint32_t j = embed_in_idx + start_idx; j < copy_buffer_size; j++) {
      // update the pointer to next token embedding data
      embed_data = (eagle_embed_in + j * increm * embedFactor);
      if (selected[j] >= 0) {
...
bool QnnNspModel::setupInput(const InferenceStep& step,
  ...
  if (!tokens.empty()) {
    ...
    if (m_modelArchitectureType == ModelArchitectureType::ENCODER) {
      const size_t pad_offset = (!m_cross_attention && (variant == ctx_size)) ? variant - n_process : 0;
      ...
      if (n_history != n_process - n_current) {
        __ERROR("setupInput : Missing token history. Expected {}, Found {}.", n_process - n_current, n_history);
        return false;
      }
  ...
  if (m_cross_attention && (m_modelArchitectureType == ModelArchitectureType::ENCODER)) {
    ...
    setupCrossAttentionMask<int32_t>(step, tokens, tokens.size() + embeddings.size()/m_embeddingBufferSize);
...
bool QnnNspModel::quantizeInput(float* in, size_t tensorOffset, size_t length) {
  ...
  switch (t_input_ids->dtype) {
    ...
    case QNN_DATATYPE_FLOAT_16:
        float32ToFloat16(reinterpret_cast<uint8_t*>(getBuffer(t_input_ids)) + tensorOffset,
                         in,
                         length);
      break;
...
double yarn_find_correction_dim(double num_rotations, size_t dim, double base, int max_position_embeddings) {
  return (dim * std::log(max_position_embeddings / (num_rotations * 2 * M_PI))) / (2 * std::log(base));
}

// Helper function: Calculate the range of frequency correction
std::pair<int, int> yarn_find_correction_range(double low_rot, double high_rot, size_t dim, double base, int max_position_embeddings) {
  ...
  if (low_dim < int_min || low_dim > int_max || high_dim < int_min || high_dim > int_max) {
    throw std::runtime_error("YaRN correction range calculation resulted in values outside valid integer range");
  }
...
bool QnnNspModel::calculate_rope_embeddings(void) {
  ...
  if (rope_scaling.rope_type == RopeScalingParams::ROPE_LLAMA3) {
    ...
    auto [low, high] = yarn_find_correction_range(beta_fast, beta_slow, dim, theta, old_context_len);
    // Generate linear interpolation mask
    std::vector<double> ramp_mask = yarn_linear_ramp_mask(low, high, half_dim);
    // Calculate frequency array
    for (size_t j = 0; j < half_dim; ++j) {
        const double pos_freq = std::pow(theta, static_cast<double>(j) / static_cast<double>(half_dim));
        ...
        inv_freq[j] = inv_freq_interpolation * (1.0 - mask_value) + inv_freq_extrapolation * mask_value;
    }
  ...
  if (rope_scaling.rope_type == RopeScalingParams::ROPE_QWEN2VL_MROPE && m_visionParam.size() > 0) {
    // Set 3d positon_ids. Refence HuggingFace
    ...
    for (size_t i = 0; i < m_visionParam.size(); i += 4) {
      ...
      // set posistion_id of pre-image text
...
bool QnnNspModel::loadTokenHistory(const std::string& loadPath) {
  ...
  file.read(reinterpret_cast<char*>(token_history.data()),
...
void QnnNspModel::setupCrossAttentionMask(const InferenceStep& step,
  ...
  if (m_mask_type == "cross-attn-row-mask") {
    ...
    if (!t_cross_attn_mask || !t_full_text_row_mask) {
      __ERROR("cross-attn-row-mask mask type requires cross_attn_mask and full_text_row_mask tensors");
      return;
    }
