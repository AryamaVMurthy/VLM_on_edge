...
int32_t EagletDialog::processFeatureVectors(Engine* targetEngine,
  ...
  if (targetEmbeddingBuffer == nullptr) {
    ...
    if (targetEmbeddingBuffer == nullptr) {
      __ERROR("EagletDialog::Required tensor '{}' or '{}' not found in target model.", targetEmbedBuffName, targetEmbedBuffNameAlt);
      return false;
    }
  }
  ...
  if (_engine.size() == 2 && !m_initFinished) {
    ...
    if (testBuffer == nullptr) {
      ...
      if (testBuffer != nullptr) {
        ...
        State::fatal(fmt::format("EagleDialog::EagleDialog tensor '{}' or '{}' not found in target model.",
                                targetEmbedBuffName, targetEmbedBuffNameAlt));
        return;
      }
    }
