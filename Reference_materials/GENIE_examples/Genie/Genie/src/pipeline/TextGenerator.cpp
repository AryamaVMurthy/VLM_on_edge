...
pipeline::TextGenerator::TextGenerator(qualla::json config,
  ...
  for (auto item : m_config.items()) {
    ...
    if (dialogConfig["dialog"]["engine"]["model"].contains("cross-attention")) {
      m_crossAttention = item.value()["engine"]["model"]["cross-attention"];
    }
    m_generator = std::make_shared<genie::Dialog>(dialogConfig, profileStat, logger);

    if (dialogConfig["dialog"].contains("engine") &&
        dialogConfig["dialog"]["engine"].contains("model") &&
        dialogConfig["dialog"]["engine"]["model"].contains("positional-encoding")) {
      qualla::json& posEncodingConfig =
          dialogConfig["dialog"]["engine"]["model"]["positional-encoding"];
...
int32_t pipeline::TextGenerator::execute(void* userData, std::shared_ptr<ProfileStat> profileStat) {
  try {
    ...
    if (!m_crossAttention) {
      ...
      useEmbedding = false;
      if (!m_generator->setCrossAttentionHiddenStates(m_pipeline->m_accumulator->getData(),
