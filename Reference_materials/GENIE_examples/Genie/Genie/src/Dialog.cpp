...
static void translateContextConfig(const qualla::json& genieConfig, qualla::json& quallaConfig) {
  if (genieConfig["dialog"].contains("context")) {
    ...
    if (genieConfig["dialog"]["context"].contains("img-token")) {
      quallaConfig["context"]["img-token"] = genieConfig["dialog"]["context"]["img-token"];
    }
    ...
    for (auto& item : config.items()) {
      if (item.key() == "rope-type") {
        ...
        if (ropeType != "llama3" && ropeType != "default" && ropeType != "longrope" &&
            ropeType != "qwen2vl-mrope" && ropeType != "yarnrope" && ropeType != "linear" && ropeType != "hunyuan") {
  ...
  for (auto& item : config.items()) {
    if (item.key() == "version") {
      ...
      if (anchorAlpha < 0.0 || anchorAlpha > 1.0) {
        throw Exception(GENIE_STATUS_ERROR_JSON_VALUE,
                        "Invalid keydiff config: anchor-alpha is not in the range [0.0, 1.0]: " + item.value().dump());
      }
    }
  }
}
...
static void translateEngineConfig(const qualla::json& genieEngineConfig,
  ...
  if (genieEngineConfig["version"] == 1) {
    ...
    if (genieEngineConfig["model"]["type"] == "binary") {
      qualla::json trimmedCtxBins = qualla::json::array();
      ...
      if (genieEngineConfig["model"]["positional-encoding"]["type"] == "rope") {
        ...
        if (genieEngineConfig["model"]["positional-encoding"].contains("rope-scaling")) {
          if (genieEngineConfig["model"]["positional-encoding"]["rope-scaling"].contains(
            ...
            if (genieEngineConfig["model"]["positional-encoding"]["rope-scaling"]["rope-type"] ==
                "yarnrope") {
              if (genieEngineConfig["model"]["positional-encoding"]
                                   ["rope-scaling"]
                                       .contains("factor")) {
                quallaEngineConfig
                    ["positional-encoding"]["rope-scaling"]["factor"] =
                        genieEngineConfig["model"]["positional-encoding"]
                                         ["rope-scaling"]["factor"];
              }
              if (genieEngineConfig["model"]["positional-encoding"]
                ...
                quallaEngineConfig["positional-encoding"]["rope-scaling"]
                                  ["dim"] =
                                      genieEngineConfig["model"]
                                                       ["positional-encoding"]
                                                       ["rope-scaling"]["dim"];
              }
              if (genieEngineConfig["model"]["positional-encoding"]
                                   ["rope-scaling"]
                                       .contains("extrapolation-factor")) {
                quallaEngineConfig["positional-encoding"]["rope-scaling"]
              ...
              if (genieEngineConfig["model"]["positional-encoding"]
                                   ["rope-scaling"]
                                       .contains("attn-factor")) {
                quallaEngineConfig
                    ["positional-encoding"]["rope-scaling"]["attn-factor"] =
                        genieEngineConfig["model"]["positional-encoding"]
                                         ["rope-scaling"]["attn-factor"];
              }
              if (genieEngineConfig["model"]["positional-encoding"]
                                   ["rope-scaling"]
                                       .contains("beta-fast")) {
                quallaEngineConfig
                    ["positional-encoding"]["rope-scaling"]["beta-fast"] =
                        genieEngineConfig["model"]["positional-encoding"]
              ...
              if (genieEngineConfig["model"]["positional-encoding"]
                                   ["rope-scaling"]
                                       .contains("beta-slow")) {
                quallaEngineConfig
                    ["positional-encoding"]["rope-scaling"]["beta-slow"] =
                        genieEngineConfig["model"]["positional-encoding"]
                                         ["rope-scaling"]["beta-slow"];
              }
              if (genieEngineConfig
                      ["model"]["positional-encoding"]["rope-scaling"]
                          .contains("original-max-position-embeddings")) {
                quallaEngineConfig["positional-encoding"]["rope-scaling"]
                                  ["original-max-position-embeddings"] =
                                      genieEngineConfig
            ...
            if (genieEngineConfig["model"]["positional-encoding"]["rope-scaling"]["rope-type"] ==
                "linear") {
              if (genieEngineConfig["model"]["positional-encoding"]["rope-scaling"].contains("factor")) {
                quallaEngineConfig["positional-encoding"]["rope-scaling"]["factor"] =
                    genieEngineConfig["model"]["positional-encoding"]["rope-scaling"]["factor"];
              }
            }
...
void Dialog::getStandaloneEnginesConfig(qualla::json& genieConfig,
  ...
  if (genieConfig["dialog"]["engine"].is_array()) {
    ...
    if (genieConfig["dialog"]["engine"]["backend"].contains("QnnGenAiTransformer") &&
      ...
      if (engineConfig["standalone-engine"]["engine"]["model"]["library"].contains("lora")) {
        qualla::json lora = engineConfig["standalone-engine"]["engine"]["model"]["library"]["lora"];
